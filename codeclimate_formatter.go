...
func codeClimatePrint(issues formatter.JSONOutput) string {
	ret := make([]CodeClimateIssue, len(issues.Issues))
	...
	codeClimateErrors := make([]CodeClimateIssue, len(issues.Errors))
	for idx, issue := range issues.Errors {
